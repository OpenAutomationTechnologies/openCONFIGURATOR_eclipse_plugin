...
public class IndustrialNetworkEditor extends MultiPageEditorPart implements
    ...
    protected void createPages() {
        try {
            ...
            for (int i = 0; i < currentProject.getNetworkConfiguration()
                    .getNodeCollection().getCN().size(); i++) {
                TCN cn = currentProject.getNetworkConfiguration()
                ...
                File file = new File(workspaceDirectory.getAbsolutePath()
                        + System.getProperty("file.separator") + "LISIM"
                ...
                public void run() {
                    ...
                    for (IWorkbenchPage page : pages) {
                        if (((FileEditorInput) editor.getEditorInput())
                            ...
                            IEditorPart editorPart = page.findEditor(editor
                                    .getEditorInput());
    ...
    void sortWords() {
        ...
        for (int i = 0; i < currentProject.getNetworkConfiguration()
            ...
            displayText.write(System.getProperty("line.separator"));
            displayText
                    .write((workspaceDirectory.getAbsolutePath()
        ...
        if (xddFiles.get(0).getISO15745Profile().get(0).getProfileBody() instanceof ProfileBodyDevicePowerlink) {
            ProfileBodyDevicePowerlink device = (ProfileBodyDevicePowerlink) xddFiles
                    .get(0).getISO15745Profile().get(0).getProfileBody();
            displayText.write(device.getDeviceIdentity().getVendorName()
                    .getValue().toString());
        }
        if (xddFiles.get(0).getISO15745Profile().get(1).getProfileBody() instanceof ProfileBodyCommunicationNetworkPowerlink) {
            ...
            displayText.write(Integer.toString(device.getApplicationLayers()
                    .getObjectList().getObject().size()));
        }
